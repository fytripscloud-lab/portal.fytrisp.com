...
@Component({
  ...
  templateUrl: './master-hotels.component.html',
  styleUrl: './master-hotels.component.scss'
})
export class MasterHotelsComponent implements OnInit {
  ...
  private performSearch(searchTerm: string) {
    if (!searchTerm) {
      this.loadHotelData(); // Reset to full list if search is empty
      return;
    }

    this.isLoading = true;
    // Assuming your API accepts a search parameter
    this.loadHotelData(1, this.pageSize, searchTerm)
  }
  ...
  loadHotelData(page: number = 1, pageSize: number = 10, search='') {
    this.isLoading = true;
    this.masterService.masterHotelList({page, page_size:pageSize, search, sort_by:this.sortColumn, sort_order:this.sortDirection, destination_id:this.destinationId, route_id:this.routeId}).subscribe({
      next: (res: any) => {
  ...
  onPageChange(event: PageEvent) {
    ...
    this.loadHotelData(pageIndex, pageSize);
  }
  ...
  loadRoutes() {
    this.isLoading = true;
    this.masterService.masterRouteList({}).subscribe({
      next: (res: any) => {
  ...
  loadRooms()
  {
    this.masterService.getAllRooms().subscribe((res:any)=>{
      this.roomLists = res.data;
    })
  }

  refresh() {
    this.loadHotelData(this.currentPage+1);
  }
  ...
  changeStatus(item_id: any, status: boolean) {
    this.alertService.confirmDialog({
      ...
      action: () => this.masterService.changeHotelStatus(item_id, status).toPromise(),
      successMessage: 'Hotel status updated successfully.'
    }).then(() => {
      this.refresh();
    }).catch((error) => {
