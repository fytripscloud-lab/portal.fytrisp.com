...
export class SuccessPaymentComponent {
  ...
  private searchSubject = new Subject<string>();
    breadscrums = [
      {
        title: 'Success Payment List',
        items: ['Home'],
        active: 'Success Payment List',
      },
    ];
    ...
    ngOnInit(): void {
      ...
      loadData(page: number = 1, pageSize: number = 10, search='') {
        this.isLoading = true;
        this.masterService.getBookingListSuccess({page, page_size:pageSize, search, sort_by:this.sortColumn, sort_order:this.sortDirection, ...this.selectParams}).subscribe({
          next: (res: any) => {
