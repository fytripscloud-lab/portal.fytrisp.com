...
export class BookingListComponent {
  ...
  searchData()
  {
    ...
    loadData(page: number = 1, pageSize: number = 10, search='') {
      this.isLoading = true;
      this.masterService.getBookingList({page, page_size:pageSize, search, sort_by:this.sortColumn, sort_order:this.sortDirection, ...this.selectParams}).subscribe({
        next: (res: any) => {
    ...
    {
      if(quotation?.payment_status=='partial'){
        this.sendQuotationPartialPayment(quotation);
        return;      
      }
      const dialogRef = this.dialog.open(SendQuotationDialogComponent, {
        ...
        dialogRef.afterClosed().subscribe((result) => {
          if (result === 1) {
            this.refresh();
            // this.viewDetails(quotation);
            this.toastr.success('Payment link send successfully.');
          }
        });
    }

    sendQuotationPartialPayment(item:any){
      Swal.fire({
        title: 'Are you sure?',
