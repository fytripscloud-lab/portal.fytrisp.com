...
export class CreateQuotationDialogComponent {  
  dialogTitle: string = 'Create Booking';
  ...
  constructor(
    ...
    if(data.action=='edit'){
      ...
      this.getPackageByDestination(this.quotation?.destination_data?.[0]?.id);
    }
  ...
  createQuotationForm(): UntypedFormGroup {    
    return this.fb.group({    
      destination_id: [this.quotation?.destination_data?.[0]?.id, [Validators.required]],
      tour_package_id: [this.quotation?.id, [Validators.required]],
