...
@Component({
  selector: 'payment-history-quotation',
  standalone: true,
...
export class PaymentHistoryComponent implements OnInit {
  ...
  private performSearch(searchTerm: string) {
    if (!searchTerm) {
      this.loadPaymentData(); // Reset to full list if search is empty
      return;
    }

    this.isLoading = true;
    // Assuming your API accepts a search parameter
    this.loadPaymentData(1, this.pageSize, searchTerm)
  }
  ...
  loadPaymentData(page: number = 1, pageSize: number = 10, search='') {
    this.isLoading = true;
    this.masterService.paymentListAll({page, page_size:pageSize, search, sort_by:this.sortColumn, sort_order:this.sortDirection, ...this.selectParams}).subscribe({
      next: (res: any) => {
  ...
  onPageChange(event: PageEvent) {
    ...
    this.loadPaymentData(pageIndex, pageSize);
  }
