...
export class UserListComponent {
  ...
  ngOnInit(): void {
    ...
    loadData(page: number = 1, pageSize: number = 10, search='') {
      this.isLoading = true;
      this.masterService.getUserLists({page, page_size:pageSize, search, sort_by:this.sortColumn, sort_order:this.sortDirection}).subscribe({
        next: (res: any) => {
    ...
    changeStatus(item_id: any, status: boolean) {
      this.alertService.confirmDialog({
        ...
        action: () => this.masterService.changeUserStatus(item_id, status).toPromise(),
