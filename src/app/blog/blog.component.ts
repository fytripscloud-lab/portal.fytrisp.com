...
export class BlogComponent implements OnInit {
  ...
  private performSearch(searchTerm: string) {
    if (!searchTerm) {
      this.loadBlogData(); // Reset to full list if search is empty
      return;
    }

    this.isLoading = true;
    this.loadBlogData(1, this.pageSize, searchTerm)
  }

  loadBlogData(page: number = 1, pageSize: number = 10, search='') {
    this.isLoading = true;
    this.masterService.getBlogList({page, page_size:pageSize, search, sort_by:this.sortColumn, sort_order:this.sortDirection}).subscribe({
      next: (res: any) => {
  ...
  onPageChange(event: PageEvent) {
    ...
    this.loadBlogData(pageIndex, pageSize);
  }
  ...
  refresh() {
    this.loadBlogData(this.currentPage+1);
  }
  ...
  changeStatus(item_id: any, status: boolean) {
    this.alertService.confirmDialog({
      ...
      action: () => this.masterService.changeBlogStatus(item_id, status).toPromise(),
  ...
  deleteBlog(itemId: string) {
    Swal.fire({
      ...
      preConfirm: () => {
        return this.masterService.deleteBlogList(itemId).toPromise()
          .catch(error => {
