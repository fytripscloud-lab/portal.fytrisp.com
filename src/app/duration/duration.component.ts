...
@Component({
  ...
  standalone: true,
  imports: [BreadcrumbComponent, DatePipe, MatIconModule, MatButtonModule, MatProgressSpinnerModule, FeatherIconsComponent, MatPaginatorModule, NgIf, MatTooltipModule],
...
export class DurationComponent implements OnInit {
  ...
  loadData(page: number = 1, pageSize: number = 10, search='') {
    this.isLoading = true;
    this.masterService.getDurationList({page, page_size:pageSize, search, sort_by:this.sortColumn, sort_order:this.sortDirection}).subscribe({
      next: (res: any) => {
        this.durationLists = res.data;   
  ...
  addNew() {
    const dialogRef = this.dialog.open(FormDialogComponent, {
      disableClose: true,
      data: {
        durationies: this.durationLists,
  ...
  editCall(row: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === 1) {
        ...
        //   'center'
        // );
        this.toastr.success('Trip duration updated successfully.');
      }
    });
  }
  ...
  changeStatus(item_id: any, status: boolean) {
    this.alertService.confirmDialog({
      ...
      action: () => this.masterService.changeDurationStatus(item_id, status).toPromise(),
      successMessage: 'Duration status updated successfully.'
    }).then(() => {
      this.refresh();
    }).catch((error) => {
