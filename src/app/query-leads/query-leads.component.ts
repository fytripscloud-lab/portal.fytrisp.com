...
@Component({
  ...
  templateUrl: './query-leads.component.html',
  styleUrl: './query-leads.component.scss'
})
export class QueryLeadsComponent implements OnInit {
  ...
  private performSearch(searchTerm: string) {
    if (!searchTerm) {
      this.loadLeadsData(); // Reset to full list if search is empty
      return;
    }

    this.isLoading = true;
    // Assuming your API accepts a search parameter
    this.loadLeadsData(1, this.pageSize, searchTerm)
  }

  loadLeadsData(page: number = 1, pageSize: number = 10, search='') {
    this.isLoading = true;
    this.masterService.getQueryLeads({page, page_size:pageSize, search, sort_by:this.sortColumn, destination_id:this.destinationId, sort_order:this.sortDirection, ...this.selectParams}, this.leadStatus).subscribe({
      next: (res: any) => {
        this.queryLeads = res.data;
      ...
      error: (error) => {       
        this.toastr.error('Error loading Query Leads');
  ...
  onPageChange(event: PageEvent) {
    ...
    this.loadLeadsData(pageIndex, pageSize);
  }
  ...
  editQuotation(row: any) {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        ...
        if(result?.error){
          this.toastr.error(result?.error?.detail);
        }
      });
  }
...
changeAction(row: any, action:string) {
  Swal.fire({
    title: "Remark for "+action,
    input: "textarea",
    inputAttributes: {
      autocapitalize: "off"
    },
    ...
    preConfirm: (text) => {
      ...
      return this.masterService.queryLeadChangeAction({
        quotation_id: row.id, 
        status: (action=='Cancel'?'Canceled':action),
        remark: text
      }).toPromise()
        .then((response) => {
          return response;
    ...
    if (result.isConfirmed && result.value) {
      this.refresh();
      this.toastr.success('Query lead updated successfully.');
    }
  });
}
